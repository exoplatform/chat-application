...
import RoomFormModal from '../../main/webapp/vue-app/components/modal/RoomFormModal';
...
function getComponent(show, selected) {
  const comp = mount(RoomFormModal, {
    propsData: {
      show: show,
      selected: selected
    },
...
describe('RoomFormModal.test.js', () => {

  it('test RoomFormModal DOM and attributes', () => {
    ...
    expect(cmp.findAll(Modal)).toHaveLength(1);
    expect(cmp.find(Modal).element.style.display).toBe('');
    cmp.vm.show = false;
    cmp.update();
    expect(cmp.find(Modal).element.style.display).toBe('none');
  });

  it('test RoomFormModal participants', () => {
  ...
  it('test RoomFormModal renderMenuItem', () => {
    const cmp = getComponent(true, {});
