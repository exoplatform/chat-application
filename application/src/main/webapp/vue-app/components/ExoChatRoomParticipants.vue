    ...
    <div :class="{collapsed: isCollapsed && mq !== 'mobile'}" class="room-participants">
      ...
      <div class="room-participants-list isList">
        <div v-for="contact in filteredParticipant" :key="contact.name" class="contact-list-item">
          <exo-chat-contact v-tiptip="contact.name" :is-external="contact.isExternalUser === 'true'" :is-enabled="contact.isEnabled === 'true' || contact.isEnabled === 'null'" :list="true" :user-name="contact.name" :name="contact.fullname" :status="contact.status" type="u"></exo-chat-contact>
        </div>
      </div>
    </div>
  </div>
