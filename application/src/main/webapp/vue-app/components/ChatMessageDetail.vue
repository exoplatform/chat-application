...
export default {
  components: { DropdownSelect, Modal },
  props: {
    miniChat: {
      type: Boolean,
      default: false
  ...
  methods: {
    ...
    executeAction(messageAction) {
      if(messageAction.confirm) {
        ...
        this.confirmAction = (message) => document.dispatchEvent(new CustomEvent(`exo-chat-message-acton-${messageAction.key}`, {detail: message}));
        this.showConfirmModal = true;
      } else {
        document.dispatchEvent(new CustomEvent(`exo-chat-message-acton-${messageAction.key}`, {detail: this.message}));
      }
    },
    editMessage(e) {
      if (
